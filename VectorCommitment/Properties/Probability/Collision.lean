...
`RO_fresh_uniform` is closed. `birthdayBound` is the remaining shared
probability bottleneck for the ROM security instances. Closing it
requires standard mathlib PMF / outer-measure / union-bound machinery
and is a real but non-novel probability-library project.

The instance files (`BindingROM`, `ExtractabilityROM`, `HidingROM`,
`EquivocationROM`) should reduce their game-specific bad events to this
bound only after their experiments are explicitly defined.
-/
